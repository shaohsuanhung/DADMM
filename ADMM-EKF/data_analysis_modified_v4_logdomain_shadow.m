...
% Plot meaning:
%   solid line : RMSE(t)
%   shadow     : log-domain RMSE(t) * exp(+/- 3 * std(log(abs(error samples))))
...
plotOpt.SemilogY            = true;      % use log-scale y-axis
...
plotOpt.YLim                = [1e-5 1e1];  % log scale cannot include 0; set [] for automatic
...
% Fill missing plot options. This keeps the code compatible with older MATLAB
% releases and allows calling plot_rmse_snr_vs_time(curves, plotOpt).
    ...
    if ~isfield(opt, 'SemilogY'),            opt.SemilogY = true; end
...
% as plot_rmse_snr_vs_time(curves, plotOpt), where plotOpt is a normal struct.
    ...
    results = struct('name', {}, 'file', {}, 'snr_dB', {}, ...
                     't', {}, 'rmse_state', {}, 'rmse_std_state', {}, 'rmse_log_std_state', {}, 'crlb_state', {});
    ...
    for i = 1:numel(curves)
        ...
        [tvec, rmse_state, rmse_std_state, rmse_log_std_state, crlb_state] = compute_rmse_crlb_vs_time( ...
        ...
        results(i).rmse_log_std_state = rmse_log_std_state;
    ...
    for s = 1:4
        ...
        for i = 1:numel(results)
            ...
            yLogStd = results(i).rmse_log_std_state(s, :).';

            % Avoid complex round-off and invalid values
            y = real(y);
            yStd = real(yStd);
            yLogStd = real(yLogStd);
            y(~isfinite(y)) = NaN;
            yStd(~isfinite(yStd)) = NaN;
            yLogStd(~isfinite(yLogStd)) = NaN;
            ...
            % Then add uncertainty shadow using the same color.
            if opt.ShowStdShadow
                if opt.SemilogY
                    % Log-domain / multiplicative error band:
                    %   yLow  = y / exp(k * std(log(abs(error))))
                    %   yHigh = y * exp(k * std(log(abs(error))))
                    factor = exp(opt.StdMultiplier * yLogStd);
                    factor(~isfinite(factor) | factor <= 0) = NaN;

                    yLow  = y ./ factor;
                    yHigh = y .* factor;

                    % Log-scale axes cannot display zero or negative values.
                    yLow  = max(yLow,  realmin('double'));
                    yHigh = max(yHigh, realmin('double'));
                else
                    % Linear-scale / additive error band.
                    band = opt.StdMultiplier * yStd;
                    yLow  = max(y - band, 0);
                    yHigh = y + band;
                end

                valid = isfinite(t) & isfinite(yLow) & isfinite(yHigh) & yLow > 0 & yHigh > 0;
                if any(valid)
                    hp = patch(ax, ...
...
function [tvec, rmse_state, rmse_std_state, rmse_log_std_state, crlb_state] = compute_rmse_crlb_vs_time( ...
...
% RMSE(t,state) is computed over all available MC/node samples.
    ...
    for t = 1:T
        ...
        if ~isempty(errSamples)
            ...
            % Additive std, used only for linear-scale shadow.
            % For one state, sqrt(error^2) = abs(error), i.e. the per-sample error magnitude.
            absErr = abs(errSamples);
            rmse_std_state(:,t) = std(absErr, 0, 2, 'omitnan');

            % Log-domain std, used for log-scale multiplicative shadow.
            % Small positive clipping avoids log(0).
            absErr = max(absErr, realmin('double'));
            rmse_log_std_state(:,t) = std(log(absErr), 0, 2, 'omitnan');
        end
