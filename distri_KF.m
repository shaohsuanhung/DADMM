...
ADMM.max_iter = 150; % Maximumconverged ADMM iterations
...
EKF.MeasureModel = @(state, idx, network_topo, env) (models_ut.LocalMeasureModel(state, idx, network_topo, env));
EKF.MeasureModelJacobian = @(state, idx, network_topo, env) (models_ut.LocalMeasureModelJacobian(state, idx, network_topo, env));
...
% EKF.system_noise = 0.01*[EKF.delta_k^4/4, 0, EKF.delta_k^3/2, 0; 
%                           0, EKF.delta_k^4/4, 0, EKF.delta_k^3/2; 
%                           EKF.delta_k^3/2, 0, EKF.delta_k^2, 0; 
%                           0, EKF.delta_k^3/2, 0, EKF.delta_k^2]; % System noise covariance
...
for mc = 1:num_monte_carlo
    ...
    for com_rad = 1:length(network_topo.com_rad_CR)
        ...
        for tar = 1: NUM_TAR
            for k = 1: TRACK_TIME
                ...
                %  if k ~= 1
                    for i = 1: network_topo.numNodes
                        ...
                        for instance = 1:NUM_CPI_PER_MEA
                            ...
                            % z_corr_from_EKFs{(k-1)*NUM_CPI_PER_MEA+instance, i} = zcorr; % TO DLELETE AFTER DEBUGGING
                            % x_corr_from_EKFs{(k-1)*NUM_CPI_PER_MEA+instance, i} =   x_corr; % TO DLELETE AFTER DEBUGGING
                            % P_corr_from_EKFs{(k-1)*NUM_CPI_PER_MEA+instance, i} =   P_corr; % TO DLELETE AFTER DEBUGGING
                            % disp("P pred: ");
                            % disp(Ppred);
                            % disp("x pred: ");
                            % disp(xpred);
                        ...
                        % TODO: And forward all the output from local EKF to ADMM. 
                        if k ~=1
                ...
                % end
                        ...
                        %     end
                        %     ADMM.prev_r{i} = r_neigbors;
                        %     ADMM.prev_v{i} = v_neigbors;
                        %     ADMM.prev_sigma_r{i} = sigma_r_neigbors;
                        %     ADMM.prev_sigma_v{i} = sigma_v_neigbors;
        ...
        if DEBUG
            ...
            hold on;
            for i = 1:network_topo.numNodes
                % plot([1:1:t], cellfun(@(x) x(1), all_estimation_from_EKFs(1:t, i)),'DisplayName', "Corr."+network_topo.labels{i});
                % plot([1:1:t], cellfun(@(x) x(1), x_pred_from_EKFs(1:t,i)), '--','DisplayName', "Pred."+network_topo.labels{i});
                %plot GT
                plot([start:1:t],  cellfun(@(x) x(1), target.target_state_wrt_node(start:t, i)),'--', 'DisplayName', "GT_"+network_topo.labels{i});
            ...
            hold on;        
            for i = 1:network_topo.numNodes
                % plot([1:1:t], cellfun(@(x) x(2), all_estimation_from_EKFs(1:t, i)), 'DisplayName', "Corr."+network_topo.labels{i});
                plot([start:1:t],  cellfun(@(x) x(2), target.target_state_wrt_node(start:t, i)),'--', 'DisplayName', "GT_"+network_topo.labels{i});
            ...
            hold on;        
            for i = 1:network_topo.numNodes
                % plot([1:1:t], cellfun(@(x) x(3), all_estimation_from_EKFs(1:t, i)), 'DisplayName', "Corr."+network_topo.labels{i});
                plot([start:1:t],  cellfun(@(x) x(3), target.target_state_wrt_node(start:t, i)),'--', 'DisplayName', "GT_"+network_topo.labels{i});
            ...
            hold on;        
            for i = 1:network_topo.numNodes
                % plot([1:1:t], cellfun(@(x) x(4), all_estimation_from_EKFs(1:t, i)), 'DisplayName', "Corr"+network_topo.labels{i});
                plot([start:1:t],  cellfun(@(x) x(4), target.target_state_wrt_node(start:t, i)),'--', 'DisplayName', "GT_"+network_topo.labels{i});
