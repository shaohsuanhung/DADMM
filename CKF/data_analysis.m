function results = plot_rmse_crlb_vs_time(curves, varargin)
...
results = struct('name', [], 'file', [], 'snr_dB', [], 't', [], 'rmse_state', [], 'crlb_state', []);
for i = 1:numel(curves)
    ...
    % select fields
    if useRaw
        estField  = pick_field(R, ["estimations_CA_raw", "estimation_CA_raw"]);
        truField  = pick_field(R, ["true_params_raw"]);
        crlbField = pick_field(R, ["CRLB"]);
    else
        estField  = pick_field(R, ["estimations_CA"]);
    ...
    [tvec, rmse_state, crlb_state] = compute_rmse_crlb_vs_time( ...
...
for s = 1:4
    ...
    for i = 1:numel(results)
        ...
        if useSemi
            ...
            h1 = semilogy(ax, t, y_rmse, '-', 'LineWidth', 1.5);
        else
            h1 = plot(ax, t, y_rmse, '-', 'LineWidth', 1.5);
        end

        % CRLB same color, dashed
        if useSemi
            h2 = semilogy(ax, t, y_crlb, '--', 'LineWidth', 1.2);
        else
            h2 = plot(ax, t, y_crlb, '--', 'LineWidth', 1.2);
        end
        h2.Color = h1.Color;
        ...
        if s == 1
            ...
            if ~isnan(results(i).snr_dB)
                labelBase = labelBase + sprintf(' (%.0f dB)', results(i).snr_dB);
            end
            hLegend(end+1) = h1; %#ok<AGROW>
            leg(end+1) = labelBase + " rMSE"; %#ok<AGROW>
            hLegend(end+1) = h2; %#ok<AGROW>
            leg(end+1) = labelBase + " CRLB"; %#ok<AGROW>
        end
    end

    ylabel(ax, sprintf('%s', stateNames{s}));
    % ylim([0 1e2])
    if s == 1
    ...
    %     set(ax,'XTickLabel',[]);
    % else
    %     xlabel(ax, useDt && ~isempty(results(1).t) && results(1).t(end) > 1 ? 'time (s)' : 'time index');
    % end
end
...
function [tvec, rmse_state, crlb_state] = compute_rmse_crlb_vs_time(L, est, tru, CRLB, stateIdx, timeDim, useDt)
...
clc;clear;close all
curves(1).name = "CKF 5db";
curves(1).file = "./data_log/test/ckf_5db/Log.mat";   % or "/mnt/data/Log.mat" if MATLAB can see that path

% curves(2).name = "CKF 10db";
% curves(2).file = "./data_log/ckf_10db/Log.mat";   % or "/mnt/data/Log.mat" if MATLAB can see that path

curves(2).name = "CKF 20db";
curves(2).file = "./data_log/ckf_20db/Log.mat";   % or "/mnt/data/Log.mat" if MATLAB can see that path

curves(3).name = "CKF 30db";
curves(3).file = "./data_log/ckf_30db/Log.mat";   % or "/mnt/data/Log.mat" if MATLAB can see that path


curves(4).name = "CKF 50db";
curves(4).file = "./data_log/ckf_50db/Log.mat";   % or "/mnt/data/Log.mat" if MATLAB can see that path

plot_rmse_crlb_vs_time(curves, 'UseRaw', true, 'SemilogY', true, 'TimeDim', 'auto');
