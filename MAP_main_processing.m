...
for mc = 1:num_monte_carol
    ...
    for com_rad = 1:length(com_rad_CR)
        ...
        while ~converged && iteration < max_iterations
            ...
            % if iteration == 1
            %     % Initialize paras
            %     range_with_error_cell = cell(1, network_topo.numNodes);
            %     ange_with_error_cell = cell(1, network_topo.numNodes);
            %     doppler_with_error_cell = cell(1, network_topo.numNodes);
            %     numNodes_cell = cell(1, network_topo.numNodes);
            %     radar_positions_cell = cell(1, network_topo.numNodes);
            %     Sigma_big_1_cell = cell(1, network_topo.numNodes);
            %     Sigma_big_2_cell = cell(1, network_topo.numNodes);
            %     %
            %     mu_r_cell = cell(1,network_topo.numNodes);
            %     mu_d_cell = cell(1,network_topo.numNodes);
            %     sigma_r_cell = cell(1,network_topo.numNodes);
            ...
            %         for j = current_neighbors
            %             k = k+1;
            %             % Put all the timestamp: M at once
            %             range_with_error_1(:,k) = range_with_error(:,j);
            %             doppler_with_error_1(:,k) = doppler_with_error(:,j);
            %             radar_positions_1(k,:) = network_topo.radar_pos(j,:);
            %             numNodes_1 = length(current_neighbors);
